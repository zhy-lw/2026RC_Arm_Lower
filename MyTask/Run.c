...
void Motor_Drive(void *param)
{
	...
	for(;;)
	{
		...
		PID_Control(Joint[3].Rs_motor.state.omega * Joint[3].inv_motor, Joint[3].pos_pid.pid_out, &Joint[3].vel_pid);
		RobStrideTorqueControl(&Joint[3].Rs_motor, Joint[3].vel_pid.pid_out * Joint[3].inv_motor);//
		
		PID_Control(Joint[4].RM_motor.actual_pos, Joint[4].exp_rad, &Joint[4].RM_motor.pos_pid);
...
void MotorSendTask(void *param)// 将电机的数据发送到PC上
{
	...
	for(;;)
	{
		...
		arm_t.joints[4].rad = Joint[4].RM_motor.actual_pos;
